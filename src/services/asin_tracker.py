...
from src.models.database import (
    TrackedAsin, AsinCurrentState, AsinHistory, 
    BestsellerChange, ApiUsageLog, ErrorLog
)
...
class AsinTracker(LoggerMixin):
    """Core service for tracking ASIN Best Seller badge changes."""
    ...
    async def get_asins_to_check(
        ...
        Returns:
            List of TrackedAsin objects ready for checking
        """
        async with get_db_session() as session:
            # Calculate cutoff time based on monitoring frequency
            now = datetime.utcnow()
            
            # Build query
            query = select(TrackedAsin).where(
                and_(
                    TrackedAsin.is_active == True,
                    or_(
                        TrackedAsin.last_checked_at.is_(None),
                        TrackedAsin.last_checked_at <= now - timedelta(
                            minutes=TrackedAsin.monitoring_frequency
                        )
                    )
                )
            ).order_by(
                TrackedAsin.priority.asc(),  # Higher priority first (1 = highest)
                TrackedAsin.last_checked_at.asc().nulls_first()  # Oldest checks first
            ).limit(limit)
            
            # Apply priority filter if specified
            if priority_filter:
                query = query.where(TrackedAsin.priority == priority_filter)
    ...
    async def process_batch(
        ...
        Args:
            tracked_asins: List of TrackedAsin objects to process
    ...
    async def _process_single_asin(
        self, 
        tracked_asin: TrackedAsin, 
        product: KeepaProductData
    ) -> List[Dict[str, Any]]:
        """
        Process a single ASIN for badge changes.
        
        Args:
            tracked_asin: TrackedAsin database object
        ...
        try:
            async with get_db_session() as session:
                ...
                current_state_query = select(AsinCurrentState).where(
                    AsinCurrentState.tracked_asin_id == tracked_asin.id
                )
                result = await session.execute(current_state_query)
                ...
                if current_state:
                    ...
                    previous_badges = [
                        BestSellerBadge(**badge) for badge in current_state.bestseller_badges
                    ]
                    ...
                    await self._update_current_state(session, current_state, product, current_badges)
                else:
                    # Create new state record
                    await self._create_current_state(session, tracked_asin, product, current_badges)
                
                # Create history record
                await self._create_history_record(session, tracked_asin, product, current_badges)
                
                # Update last_checked_at
                tracked_asin.last_checked_at = datetime.utcnow()
                await session.merge(tracked_asin)
    ...
    async def _create_change_record(
        ...
        """Create a bestseller change record in the database."""
        
        change = BestsellerChange(
            tracked_asin_id=tracked_asin.id,
            asin=tracked_asin.asin,
    ...
    async def _send_change_notification(
        ...
        try:
            payload = SlackNotificationPayload(
                asin=tracked_asin.asin,
                product_title=product.title or tracked_asin.product_title or f"Product {tracked_asin.asin}",
            ...
            if success:
                ...
                async with get_db_session() as session:
                    await session.execute(
                        update(BestsellerChange)
                        .where(BestsellerChange.id == change["id"])
                        .values(
    ...
    async def _update_current_state(
        self,
        session: AsyncSession,
        current_state: AsinCurrentState,
        product: KeepaProductData,
        badges: List[BestSellerBadge]
    ) -> None:
        """Update existing current state record."""
        
        current_state.bestseller_badges = [badge.dict() for badge in badges]
        current_state.sales_ranks = product.sales_ranks or {}
        current_state.category_tree = product.category_tree or []
        current_state.product_title = product.title
        current_state.brand = product.brand
        current_state.current_price = product.current_price
        current_state.availability_amazon = product.availability
        current_state.monthly_sold = product.monthly_sold
        current_state.keepa_last_update = product.last_update
        current_state.raw_keepa_data = product.dict()
        current_state.updated_at = datetime.utcnow()
        
        await session.merge(current_state)
    ...
    async def _create_history_record(
        ...
        """Create history record for this check."""
